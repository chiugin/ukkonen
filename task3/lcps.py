...
class Tree:
    ...
    def get_lcps(self,node,i,j,string,totalLen):
        """
        obtains the longest common prefix of suffix strings beginning at position i and j
        """

        # in the beginning (of the first recursion), node is referring to root node cuz it's the starting node
        if string[i+totalLen] == string[j+totalLen]:
            childnode = node.children[ord(string[j+totalLen])]
            lenEdge = childnode.end.end - childnode.start + 1
            totalLen += lenEdge
            return self.get_lcps(childnode,i,j,string,totalLen)
        else:
            return totalLen
...
if __name__ == "__main__":
    ...
    for line in fpat:
        ...
        lcp = suffixTree.get_lcps(root,int(i)-1,int(j)-1,text+"$",0)
        lcp = str(lcp)
        output = i+" "+j+" "+lcp+"\n"
        fout.write(output)
